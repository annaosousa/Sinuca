...
from collisions import check_if_ball_touches_ball
...
class GameState:
    ...
    def game_event_handler(self, event):
        if event.type == "POTTED":
            ...
            if not self.white_ball_1st_ht_is_set:
                self.first_collision(event.data)

    def set_pool_balls(self):
        count = [0,0]
        coord_shift = np.array([math.sin(math.radians(60)) * config.white_ball_radius * 2, - config.white_ball_radius])
    ...
    def reset_state(self):
        ...
        self.potted = []
        self.balls = pygames.sprite.Group()
    ...
    def redraw_all(self, update=True):
        self.all_sprites.clear(self.canvas.surface, self.canas.background)
    ...
    def game_over(self, p1_won):
        ...
        if p1_won:
            ...
            text="PLAYER 2 WON"
        render_text = font.render(text,false,(255,255,255))
    ...
    def potted_ball_rules(self):
        if len(self.potted) > 0:
            ...
            only_solid_potted = potted_stripe_count == 0 and potted_solid_count > 0

            if only_solid_potted or only_stripes_potted:
                selected_ball_type = BallType.Stripe if only_stripes_potted else BallType.Solid
