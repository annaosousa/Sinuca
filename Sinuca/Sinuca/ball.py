...
class Ball():
    ...
    def apply_force (elf, force, time=1):
        self.veocity = (force/ config.ball_mass) * time

    def set_velocity(self, new_velocity):
        self.velocity = np.array(new_velocity, dtype=float)

    def move_to(self, pos):
        self.pos = np.array(new_velocity, dtype=float)
...
class BallSprite(pygame.sprite.Sprite):
    ...
    def update(self, *args):
        if np.hypot(*self.ball.velocity) != 0:
            ...
            transformation_matrix = physics.rotation_matriz(perpendicular_velocity,rotation_angle)
    ...
    def update_sprite(self):
        ...
        label_demension = np.repeat([self.label_size *2], 2)
        label = pygame.Surface(label_demension)
        label_demension.fill(self.color)
        ...
        if self.label_offset[2] > 0:
            ...
            if self.label_offset[0] != 0:
                ...
                label = pygame.transform.scale(label, (int(config.ball_radius * dist_from_center), config.ball_radius))
        ...
        grid_2d = np.mgrid[-config.ball_radius:config.ball_radius + 1, -config.ball : config.ball_radius + 1]
        ...
        self.top_left = self.ball.pos - config.ball_radius
        self.rect.centre = self.ball.pos.tolist()
        ...
        def is_active(self, game_state, behind_separation_line = False):
            ...
            if np.all(np.less(config.table_margin + config.ball_radius + config.hole_radius, events["mouse_pos"])) and\
                np.all(np.greater(config.resolution - config.table_margin - config.ball_radius - config.hole_radius, events["mouse"])) and\
                not collisions.check_if_ball_touches_ball(events["mouse_pos"], self.number, game_state.balls):
