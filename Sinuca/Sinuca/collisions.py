...
def resolve_all_collisions(balls,holes,tables_sides):
    ...
    for line_ball_combination in itertools.product(table_sides, balls):
        if physics.line_ball_combination_check(line_ball_combination[0], line_ball_combination[1].ball):
...
def check_if_ball_touches_ball(target_ball_pos, target_ball_number, balls):
